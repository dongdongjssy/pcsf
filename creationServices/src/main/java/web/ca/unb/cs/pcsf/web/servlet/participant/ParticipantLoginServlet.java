...
import static ca.unb.cs.pcsf.web.PCSFWebConstants.ATTRIBUTE_PARTICIPANT_BEAN;
import static ca.unb.cs.pcsf.web.PCSFWebConstants.ATTRIBUTE_PARTICIPANT_MSG;
import static ca.unb.cs.pcsf.web.PCSFWebConstants.COMMON_PAGE_ERROR;
import static ca.unb.cs.pcsf.web.PCSFWebConstants.MESSAGE_RESULT_FAIL;
import static ca.unb.cs.pcsf.web.PCSFWebConstants.MESSAGE_RESULT_OK;
import static ca.unb.cs.pcsf.web.PCSFWebConstants.MSG_PARTICIPANT_INVOLVED_COLLABORATION;
...
import ca.unb.cs.pcsf.web.PCSFMessage;
...
public class ParticipantLoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (message.getObject() == null) {
			...
			Collaboration collaboration = dbAccessor.getCollaborationById(participant
					.getCollaborationId());

			session.setAttribute(ATTRIBUTE_PARTICIPANT_MSG,
					MSG_PARTICIPANT_INVOLVED_COLLABORATION);
	...
	private PCSFMessage loginAsParticipant(String participantId) {
		...
		if (dbAccessor.isParticipantExist(participantId)) {
			...
			return new PCSFMessage(MESSAGE_RESULT_OK, "Participant "
					+ participant.getName() + " login successfully!", participant);
		} else {
			logger.info("Participant doesn't exist!");
			logger.debug(LOGPRE + "loginAsParticipant() end" + LOGPRE);
			return new PCSFMessage(MESSAGE_RESULT_FAIL, "Participant doesn't exist!",
					null);
		}
	}
}
