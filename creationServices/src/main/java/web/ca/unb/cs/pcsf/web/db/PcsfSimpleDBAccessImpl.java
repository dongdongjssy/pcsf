...
import static ca.unb.cs.pcsf.services.crt.CreationServiceConstants.MAIL_CONTENT_PARTICIPANT_REG_REQUIRE;
...
public class PcsfSimpleDBAccessImpl implements PcsfSimpleDBAccess {
  ...
  public void putDataIntoDomain(Object object) {
    ...
    if (object instanceof Participant) {
      ...
      participant.setId(idGenerator(DOMAIN_PARTICIPANT));
      items.add(new ReplaceableItem(participant.getId()).withAttributes(new ReplaceableAttribute(
          ...
          PARTICIPANT_ATTRIBUTE_COLLABORATION_ID, participant.getCollaborationId(), false), new ReplaceableAttribute(
          PARTICIPANT_ATTRIBUTE_IS_REG, participant.getIsReg(), true), new ReplaceableAttribute(
          PARTICIPANT_ATTRIBUTE_ROLE, participant.getRole(), true), new ReplaceableAttribute(
          PARTICIPANT_ATTRIBUTE_GROUP, participant.getGroup(), true)));

      sendParticipantNotificationMail(participant.getEmail(), participant.getName(), participant.getId());
    ...
    if (object instanceof Creator) {
      ...
      if (!isCreatorExist(creator.getName())) {
        List<ReplaceableItem> items = new ArrayList<ReplaceableItem>();
        items.add(new ReplaceableItem(creator.getId()).withAttributes(new ReplaceableAttribute(CREATOR_ATTRIBUTE_NAME,
            creator.getName(), true),
            new ReplaceableAttribute(CREATOR_ATTRIBUTE_PASSWORD, creator.getPassword(), true),
    ...
    if (object instanceof Collaboration) {
      ...
      if (!isCollaborationExist(collaboration.getName())) {
        ...
        item.withAttributes(new ReplaceableAttribute(COLLABORATION_ATTRIBUTE_NAME, collaboration.getName(), true),
            new ReplaceableAttribute(COLLABORATION_ATTRIBUTE_CREATOR_ID, collaboration.getCreatorId(), true),
            new ReplaceableAttribute(COLLABORATION_ATTRIBUTE_CURRENT_STATE, collaboration.getCurrentState(), true),
            new ReplaceableAttribute(COLLABORATION_ATTRIBUTE_WORKFLOW_MODEL, collaboration.getWorkflowModel(), true));

        List<Participant> participants = collaboration.getParticipants();
        for (Participant participant : participants)
          item.withAttributes(new ReplaceableAttribute(COLLABORATION_ATTRIBUTE_PARTICIPANT, participant.getName(), true));
  ...
  public void update(Object o) {
    ...
    if (o instanceof Collaboration) {
      ...
      replaceableAttributes.add(new ReplaceableAttribute(COLLABORATION_ATTRIBUTE_NAME, collaboration.getName(), true));
      for (Participant s : collaboration.getParticipants())
        replaceableAttributes.add(new ReplaceableAttribute(COLLABORATION_ATTRIBUTE_PARTICIPANT, s.getName(), true));
    ...
    if (o instanceof Participant) {
      ...
      replaceableAttributes.add(new ReplaceableAttribute(PARTICIPANT_ATTRIBUTE_IS_REG, participant.getIsReg(), true));
   ...
   * (non-Javadoc)
   * 
   * @see ca.unb.cs.pcsf.db.DBAccessService#updateCollaborationState(ca.unb.cs.pcsf.db.Collaboration, java.lang.String)
   */
  ...
  public Creator getCreatorByName(String userName) {
    ...
    String selectRequest = "select * from `" + DOMAIN_CREATOR + "` where " + CREATOR_ATTRIBUTE_NAME + " = '" + userName
        + "'";
  ...
  private void sendParticipantNotificationMail(String pEmail, String pName, String pId) {
    ...
    try {
      ...
      newMessage.setText("Dear " + pName + ",\n\n" + MAIL_CONTENT_PARTICIPANT_REG_REQUIRE + "Your id: " + pId + "\n"
          + LINK_PARTICIPANT);
