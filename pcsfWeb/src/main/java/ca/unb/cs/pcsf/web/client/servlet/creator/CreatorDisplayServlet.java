...
import static ca.unb.cs.pcsf.web.client.PCSFWebClientConstants.ATTRIBUTE_ADDED_PARTICIPANTS;
...
import static ca.unb.cs.pcsf.web.client.PCSFWebClientConstants.ATTRIBUTE_CREATOR_DISPLAY_MSG;
import static ca.unb.cs.pcsf.web.client.PCSFWebClientConstants.ATTRIBUTE_CREATOR_NAME;
import static ca.unb.cs.pcsf.web.client.PCSFWebClientConstants.MSG_COLLABORATION_CREATED;
import static ca.unb.cs.pcsf.web.client.PCSFWebClientConstants.MSG_NO_COLLABORATION_CREATED;
...
public class CreatorDisplayServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<Collaboration> collaborations = dbAccess.getCollaborationsByCreatorId(creator.getId());

		session.setAttribute(ATTRIBUTE_CREATOR_NAME, creator.getName());
		if (collaborations == null) {
			session.setAttribute(ATTRIBUTE_CREATOR_DISPLAY_MSG, MSG_NO_COLLABORATION_CREATED);
		} else {
